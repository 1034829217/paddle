...
class GraphConvLayer(nn.Layer):
    ...
    def __init__(self, input_dim=4096, output_dim=1024, dropout=0.1, negative_slope=0.2, bias = True):
        ...
        self.ac = nn.LeakyReLU(negative_slope)
        self.bias = bias

        self.weight = paddle.create_parameter(shape=[output_dim, input_dim], dtype='float32')
        initializer.KaimingUniform(self.weight, negative_slope=np.math.sqrt(5))
        self.bias = None
        # if bias:
    ...
    def forward(self, x, adj=None, adjflag=False):
        ...
        view1_feature = paddle.mm(x, self.weight.t())
