...
def calc_loss(view1_feature, view2_feature, view1_predict, view2_predict, labels_1, labels_2, gama=0.5):
    ...
    term1 = ((view1_predict - labels_1.astype('float32')) ** 2).sum(1).sqrt().mean() + (
                (view2_predict - labels_2.astype('float32')) ** 2).sum(1).sqrt().mean()
...
def calc_loss2(view1_feature, view2_feature, view1_predict, labels_1, tau=12):
    ...
    term1 = ((view1_predict - labels_1.astype('float32')) ** 2).sum(
        1).sqrt().mean()
...
def train_gse(model, data_loaders, optimizer, num_epochs=100):
    ...
    # paddle.save(model.state_dict(), "save/pre-model.pt")
...
def train_gan(model_genT, model_disT, model_gen, model_dis, model, data_loaders, optimizer_genT, optimizer_disT,
            ...
            for imgs, txts, labels in data_loaders[phase]:
                ...
                with paddle.set_grad_enabled(phase == 'train'):
                    ...
                    if True:
                        ...
                        dloss = - (paddle.log(score_r) + paddle.log(1 - score_f))
                    ...
                    if True:
                        ...
                        gloss = -paddle.log(score_f) + 1.2 * paddle.log(
                            predloss)
                    ...
                    ######################
                    if True:
                        optimizer_disT.clear_grad()
                        gentxt = model_genT(txts)
                        score_f = model_disT(gentxt)
                        genimg = model_gen(imgs)
                        score_r = model_disT(genimg)

                        dlossT = - (paddle.log(score_r) + paddle.log(1 - score_f))
                    ...
                    if True:
                        ...
                        glossT = -paddle.log(score_f) + 1.0 * paddle.log(
                            predloss)
    ...
    # paddle.save(model_gen.state_dict(), "save/pre-model_gen.pt")
    # paddle.save(model_genT.state_dict(), "save/pre-model_genT.pt")
