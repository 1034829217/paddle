...
from models.VDSH_SP_paddle import VDSH_SP
import argparse
...
#########################################################################################################
model = VDSH_SP(dataset, num_features, num_bits, y_dim, device=device, dropoutProb=args.dropout, use_softmax=args.single_label)
# model.to(device)
...
with open('logs/VDSH_SP/loss.log.txt', 'w') as log_handle:
    ...
    for epoch in range(num_epochs):
        ...
        for step, (xb, yb) in enumerate(train_loader):
            ...
            pred_weight = min(pred_weight + pred_weight_step, 250.)
            avg_loss.append(loss.item())
            
            log_handle.write('{},{},{:.4f},{:.4f},{:.4f},{:.4f}'.format(epoch, step, loss.item(), 
                                                                 reconstr_loss.item(), z_kl_loss.item(), v_kl_loss.item()))
...
#########################################################################################################
with open('logs/VDSH_SP/result.txt', 'a') as handle:
